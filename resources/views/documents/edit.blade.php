            ...
            <div class="bg-white p-6 rounded-lg shadow">
                <form method="POST" action="{{ route('documents.update', $document) }}" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="project_name" value="{{ old('project_name', $document->project_name) }}" class="mt-1 w-full rounded-lg border-gray-300" required>
                    ...
                    <div>
                        ...
                        <input name="title" value="{{ old('title', $document->title) }}" class="mt-1 w-full rounded-lg border-gray-300" required>
                    ...
                    <div>
                        ...
                        <select name="type" class="mt-1 w-full rounded-lg border-gray-300" required>
                            @foreach([
                                ...
                                <option value="{{ $k }}"
                                    @selected(old('type', $document->type) === $k)>
                                    {{ $v }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3" class="mt-1 w-full rounded-lg border-gray-300">{{ old('notes', $document->notes) }}</textarea>
                        @error('notes') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>

                    <div class="rounded-lg bg-gray-50 p-3 border">
                        <div class="text-sm font-medium">File saat ini:</div>
                        <div class="text-sm text-gray-700">{{ $document->original_name }}</div>
                        <a class="text-sm text-indigo-600 hover:underline" href="{{ route('documents.download', $document) }}">Download</a>
                    </div>

                    <div>
                        <label class="block text-sm font-medium">Ganti File (opsional)</label>
                        <input type="file" name="file" class="mt-1 w-full">
                        <div class="text-xs text-gray-500 mt-1">Kalau tidak diisi, file tidak berubah.</div>
                        @error('file') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>

                    <div class="flex gap-2">
                        <button class="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-500">Save</button>
